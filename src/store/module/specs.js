...
const actions={
    ...
    changeSpecsList(context,bool){
        ...
        reqSpecsList(params).then(res=>{
            ...
            if(context.state.page!=1 && list.length==0){
                ...
                // 再次请求list
                context.dispatch('changeSpecsList');
                return
            }
            // 处理list里attrs中的数据
    ...
    // 请求总数
    changespecsTotal(context){
        reqSpecsTotal().then(res=>{
            // console.log(res);
            context.commit("specsTotal",res.data.list[0].total)
        })
    },
    // 请求修改当前页码
